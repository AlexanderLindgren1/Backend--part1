...
async function start() {
  while (appActive) {
    ...
    //Adding a new movie
      ...
      moviesCol.insertOne({
        ...
        ratings: chooseRatings
          ? chooseRatings.split(",").map((n) => Number(n))
          : [],
      ...
      if (chosenMovie) {
        ...
        if (updateField == 1 || updateField == 2 || updateField == 3) {
          ...
          if (AddOrDel == 1) {
            ...
            let Adding = p(
              "what do you want to add (Will only add one)?"
            );
            if (updateField == "4") {
              await moviesModel.updateOne(
                { _id: chosenMovie._id },
    ...
    //delete collection that user choose
    else if (input == "4") {
